...
export class Helper {
    ...
    getStyleAPI(uri): string {
        let apiDomain = (window as any).API_DOMAIN;

        if (apiDomain && apiDomain.substring(0, 4) == 'http' || apiDomain && apiDomain.substring(0, 5) == 'https') {
            return apiDomain + '/style/' + uri;
        } else {
            return (<any> window).location.href + 'style/' + uri;
        }
    }
